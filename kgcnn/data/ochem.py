...
class OCHEMDataset(MemoryGraphDataset):
    ...
    def prepare_data(self, overwrite: bool = False, smiles_column_name: str = "smiles",
        ...
        if os.path.exists(self.file_path_mol) and not overwrite:
            ...
            self.cols = [col for col in self.data_frame.columns if col.startswith('Result')]
            self.descs = [col for col in self.data_frame.columns if col.startswith('desc')]
            self.info("Found SDF %s of pre-computed structures." % self.file_path_mol)
            return self

        self.read_in_table_file()
        smiles = self.data_frame[smiles_column_name].values
        # this is the graph_labels == targets and molecules properties (additional values to concate to the latent-space!)
        self.cols = [col for col in self.data_frame.columns if col.startswith('Result')]
        self.descs = [col for col in self.data_frame.columns if col.startswith('desc')]
    ...
    def set_attributes(self,
        ...
        if label_column_name:
            ...
            callbacks.update({'graph_labels': lambda mg, ds: ds[self.cols]})

        # maybe a problem of casts!
        ...
        callbacks.update({
            ...
            'graph_attributes': lambda mg, ds: np.hstack((np.array(mg.graph_attributes(graph, encoder_graph), dtype='float32'), np.array(ds[self.descs], dtype='float32')))
        })
